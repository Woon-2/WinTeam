...
{
	if (!MapPixelCollision(terrain_dc, RGB(255, 0, 0), POINT{ pos.x - x_move_px, pos.y + height / 2 }))
		return true;
	return false;
}
...
{
	if (!MapPixelCollision(terrain_dc, RGB(255, 0, 0), POINT{ pos.x + width + x_move_px, pos.y + height / 2 }))
		return true;
	return false;
}
...
{
	if (MapPixelCollision(terrain_dc, RGB(0, 255, 0), POINT{ pos.x,  pos.y + height }))
		return true;
	return false;
}
